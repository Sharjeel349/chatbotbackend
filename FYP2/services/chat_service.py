...
def save_chat_message(session_id: int, sender: str, message_text: str, voice_file: str,emotion: str ,db_conn):
    cur = db_conn.cursor()
    try:
        query = """
            INSERT INTO Chat_Message (session_id, sender, message_text, voice_file,emotion)
            VALUES (%s, %s, %s, %s,%s)
            RETURNING message_id, sender, message_text, timestamp,emotion;
        """
        cur.execute(query, (session_id, sender, message_text, voice_file,emotion))
